...
export function generateHTMLReportContent(
  ...
  if (reportType === 'student_progress') {
    ...
    body = `<div class="metrics"><div class="metric"><span>Kazanılan</span><strong>${esc(formatCurrency(earned,teacher.currency))}</strong></div><div class="metric"><span>Tahsil Edilen</span><strong>${esc(formatCurrency(collected,teacher.currency))}</strong></div><div class="metric"><span>Fark</span><strong>${esc(formatCurrency(earned-collected,teacher.currency))}</strong></div></div><h2>Hesap Hareketleri</h2>${table(['Tarih','Öğrenci','İşlem','Açıklama','Yöntem','Tutar'], [...filtered].filter(t=>!t.isCancelled).sort((a,b)=>b.date.localeCompare(a.date)).map(t => [esc(formatDateTurkish(t.date,'short')), esc(studentName(t.studentId, students)), esc(t.type), esc(t.description), esc(t.paymentMethod || '-'), `<strong>${esc(formatCurrency(t.amount,teacher.currency))}</strong>`]))}`;
  } else {
    title = 'Genel Rapor';
